...
///   logging      -> JSON access log to the shared volume (Matcat ingests it)
///   apps.http    -> one route per enabled Matcat route (host matcher +
...
public class CaddyConfigGenerator
{
    ...
    public string AccessLogPath =>
        (_cfg["Matcat:CaddyLogDir"] ?? "/caddy-logs").TrimEnd('/') + "/access.log";

    /// <summary>The URL Caddy uses to reach Matcat for forward-auth verification.</summary>
    private string MatcatUpstream => _cfg["Matcat:SelfUpstream"] ?? "matcat:4433";
        ...
        var config = new Dictionary<string, object?>
        {
            ...
            ["logging"] = new Dictionary<string, object?>
            {
                ["logs"] = new Dictionary<string, object?>
                {
                    ["access"] = new Dictionary<string, object?>
                    {
                        ...
                        ["include"] = new[] { "http.log.access.matcat" }
                    }
                }
            },
            ["apps"] = new Dictionary<string, object?>
            {
                ["http"] = new Dictionary<string, object?>
                {
                    ...
                    {
                        ["matcat"] = new Dictionary<string, object?>
                        {
                            ["listen"] = new[] { ":80", ":443" },
                            ["routes"] = httpRoutes,
                            // Route this server's access logs to the "matcat"
                            // logger so the custom file log below captures them.
                            ["logs"] = new Dictionary<string, object?>
                            {
                                ["default_logger_name"] = "matcat"
                            }
                        }
                    }
                }
    ...
    private object BuildTerminalHandler(RouteConfig route)
    {
        ...
        return new Dictionary<string, object?>
        {
            ["handler"] = "static_response",
            ["status_code"] = 503,
            ["body"] = "Matcat: no upstream configured for this route."
        };
    }

    /// <summary>
    /// Auth handler chain for a route. BasicAuth -> Caddy http_basic.
    /// Matcat -> forward_auth style verification against Matcat's /auth/verify
    /// (fully wired in M6).
    /// </summary>
    private IEnumerable<object> BuildAuthHandlers(AuthenticationConfig auth)
    {
        ...
        else if (auth.Type == AuthType.Matcat)
        {
            // forward_auth: ask Matcat to verify the session; on 2xx continue,
            // otherwise Matcat responds with a redirect to its login portal.
            yield return new Dictionary<string, object?>
            {
                ["handler"] = "reverse_proxy",
                ["upstreams"] = new[] { new Dictionary<string, object?> { ["dial"] = MatcatUpstream } },
                                ...
                                ["handle"] = new[]
                                {
                                    new Dictionary<string, object?>
                                    {
                                        ...
                                        ["request"] = new Dictionary<string, object?>
                                        {
                                            ["set"] = new Dictionary<string, object?>
                                            {
                                                ["X-Matcat-User"] = new[] { "{http.reverse_proxy.header.X-Matcat-User}" }
                                            }
                                        }
                                    }
                                }
    ...
    private object? BuildTls(List<RouteConfig> routes, MatcatSettings settings)
    {
