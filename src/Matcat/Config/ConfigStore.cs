...
public class ConfigStore
{
    ...
    private MatcatSettings? _settings;

    public ConfigStore(IConfiguration cfg, ILogger<ConfigStore> log)
    {
        _log = log;
        _dir = cfg["Matcat:ConfigDir"] ?? "/data/config";
    ...
    public MatcatSettings Settings
    {
        get
        {
            lock (_lock)
            {
                ...
                if (File.Exists(path))
                {
                    try { _settings = JsonSerializer.Deserialize<MatcatSettings>(File.ReadAllText(path), JsonOpts); }
                    catch (Exception ex) { _log.LogError(ex, "Failed to read settings.json"); }
                }
                return _settings ??= new MatcatSettings();
            }
        }
    }
    ...
    public void SaveSettings(MatcatSettings settings)
    {
        lock (_lock)
        {
            _settings = settings;
